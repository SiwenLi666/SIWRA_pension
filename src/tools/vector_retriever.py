    ...
    def _generate_response(self, question: str, documents: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                ...
                paragraph = ", ".join(metadata.get("paragraphs", [])) if isinstance(metadata.get("paragraphs", []), list) else ""
                page_numbers = metadata.get("pages", [])
                ...
                if chapter:
                    ref_parts.append(f"kapitel: {chapter}")
                if paragraph:
                    ref_parts.append(f"paragraf: {paragraph}")
...
{references_text}"""
            ...
            for ref in references:
                # Extract the reference ID and content
            ...
            # Add the references as HTML to the response
